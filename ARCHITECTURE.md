...
| Concern | Design |
|---|---|
| **Filtering** | `resolve_recipients(p_filter jsonb) RETURNS TABLE (person_id, email, merge jsonb)`. Accepts every axis the PRD names: `term_ids[]`, `join_years[]`, `roles[]`, `region_ids[]`, `island_groups[]`, `affiliation_ids[]`, `statuses[]` — **plus, since 2026-09-06 (migration `0047`), `department_ids[]`, `committee_ids[]`, `university_ids[]` and `year_levels[]`**, so the picker below reaches the same axes CRRD already uses to browse members. `join_year` from `people.join_year` (the PRD's "year of membership"); `island_group` from the regions join; affiliation from `member_affiliations` — so **"START x DataCamp" is a row, and a new partnership needs no code change**. Compiles to a parameterized query, never string concatenation. |
| **Hand-picking** *(2026-09-06, migration `0047`)* | The filter axes narrow a list; `select_all` (default `true`), `person_ids` and `excluded_person_ids` say who from it actually gets sent to: `recipients = (select_all ? filter-matches : ∅) ∪ person_ids − excluded_person_ids`. A hand-added person still has to clear the same eligibility bar as everyone else — a current-term membership, an email on file, not redacted — even if the filter axes wouldn't have matched them. The composer's picker reads this through `list_audience_candidates(p_filter, p_q, p_limit, p_offset)`, a **second** `SECURITY DEFINER` function guarded exactly like `resolve_recipients()` (`crrd_admin`/`exec_admin` only, `revoke execute from public, anon`) — it applies the filter axes only (never `select_all`/`person_ids`, which are selection state the client overlays) and returns name, member ID, region, department, committee and position, **never an address**, so widening the picker can never widen what a bulk send is allowed to merge. |
...
| **Token safety** | Each template declares merge variables as a zod schema. Substitution is a strict `{{token}}` replace that **THROWS on an unknown token** rather than shipping a literal `{{frist_name}}` to 600 scholars. **Any** `{{…}}` that is not a whitelist field counts as unknown, including the spaced `{{First Name}}` spelling other mail-merge tools use — an earlier scan matched only `[a-zA-Z0-9_]+` and let that one through untouched, which is the one way a placeholder could still reach a mailbox as literal text (fixed 2026-09-07, ADR 0014 §7). A token inside an HTML **attribute** is refused rather than escaped: escaping that is right for text is not sufficient inside a URL. Every value is HTML-escaped, so a name containing markup cannot inject into another recipient's mail. |
...
| **Templates** | Three shipped: `MembershipApplicationInvite`, `MembershipRenewal`, and `Freeform` (rich-text body CRRD edits in-app). Templates are code-reviewed — a template is the one place a merge field could leak someone else's data. **`CommitteeCall` was removed 2026-09-11 (QA finding CAMPAIGNS-04):** it pre-filled a link to `/committee-apply`, which has never existed, so recipients hit the login wall — and members hold no accounts. The committee form is v1.1 (PRD item 24 / US-G6); the template returns in the PR that ships the route. `lib/campaigns/templates.test.ts` asserts every template's link is a real public route. |
...
**Interim implementation (2026-09-05, migration `0043`, ADR 0010) — where the shipped code departs from the sketch above, stated so nobody debugs the diagram.** (1) The transport is **Gmail SMTP** behind `lib/mail/` (`MAIL_TRANSPORT=gmail_smtp|fake`), not Resend — the org has no verified domain yet (OQ-10); Resend is an env flip when it does. (2) `resolve_recipients()` is **SECURITY DEFINER**, guarded to `crrd_admin` and `exec_admin`: the `0015` column GRANT withholds `people.personal_email` from every session, so an INVOKER function could never read an address. Regional-rep sending (`rr_send_grants`) is not built. (3) The drain is **25 per step, driven by the CRRD's browser** (`drainCampaign` in a loop from the campaign page) rather than a self-invoking function plus a 15-minute sweep — no sweep exists yet; a chunk that dies leaves rows leased for ten minutes and the next click resumes them. (4) **No delivery webhooks**: the report is sent/failed only; a bounce lands in the sending inbox (ADR 0010). (5) The body is **either** Telegram-style markdown rendered to escaped HTML by our own renderer (`lib/campaigns/markdown.ts`) **or**, since 2026-09-07 (migration `0049`, ADR 0014), a designed template pasted from an email builder and run through the server-side allowlist in `lib/campaigns/sanitize.ts` — `email_campaigns.body_format` says which. The markdown path is escape-first, so only our own tags reach a mail client; the pasted path keeps only what the allowlist names, drops `<script>`/`<style>`/`<iframe>`/`<form>` with their contents, filters inline CSS one property at a time, and permits `https`, `mailto` and `tel` only. **The sanitiser runs inside the Server Action and its output is what is stored** — the raw paste is never persisted and never sent, and the composer previews it in a sandboxed iframe so the CRRD sees exactly what was kept. The database does not sanitise and does not claim to: `0049`'s CHECK constraints are size backstops (200 KB of source, 256 KB of rendered body), and ADR 0014 records the residual gap. `v_email_merge_fields` and the strict `{{token}}` merge are exactly as specified.
...
**Membership end** (graduate / resign / leave) is a separate, per-person status change: the Server Action sets `memberships.status` and in the same action calls `auth.admin.signOut(user_id, 'global')`. The member portal's RLS requires an active membership in `current_term_id()`. The sole carve-out is a policy on `renewal_submissions` permitting a person with **any** past membership to insert exactly one renewal row while a renewal window is open. **Shipped differently (2026-09-06, migration `0044`, SRS 2026-09-05): members hold no accounts, so the renewal form at `/renew` is accountless.** The scholar presents their member ID and the email on file; `start_renewal()` (SECURITY DEFINER, anon-callable, window-gated on `form_kind = 'membership_renewal'`) resolves the person from that pair, refuses a terminated member (CBL Art. VII §3) or one already active this term, and writes the draft; the two documents follow the `/apply` path (direct PUT, server-side re-verification, `finalize_renewal()` behind a submit token); `approve_renewal()` inserts the new term's membership row and never touches `member_id`. There is still no INSERT or UPDATE policy on the table for any human role — the member-side policy this paragraph describes was never written, and the accountless form replaces it. The PRD's "no access to any part of the website unless it's for membership renewal" is therefore **two policies, not an if-statement someone can forget to write.**
...
> **Amended 2026-09-05 — the CRRD SRS role tiers (migration `0036`, ADR 0009).** The CCDO's "Questions Roles and Features" document and the team meeting of the same day set the tiers by department: **CEO & COO** → `exec_admin`; **CTO & DCTO-PD** → `tech_admin`; **CCDO, DCCDO-C, DCCDO-D** → `crrd_admin`; every other Chief and Deputy → `officer`; Regional Representatives → `regional_rep`; **members hold no accounts** ("Members cannot access the system. They can only submit via forms"). There is **no Moderator tier** ("there are no moderator roles listed in the SRS"): the enum label stays because Postgres cannot drop one, but `user_roles_no_retired_tier` refuses it. `member` survives only as the **revoked** state `revokeRole` writes — it reaches no route. Administrators are therefore **seven**, asserted by the `admin_is_srs_administrator` CHECK. This reverses the project-head split of 2026-09-01; where the text below still describes that split, this note wins.

The **Who** column below is the Constitution's, not ours: every holder is a position seeded from CBL Art. III §2 (Executive Board), §3 (Deputy Board), §4.6 (Regional Representatives) or §5 (Committee Members), with `officer_positions.grants_org_role` mapping title → capability (DATA_MODEL.md §6/0016). **Administrators are exactly seven: CEO, COO, CTO, DCTO-PD, CCDO, DCCDO-C, DCCDO-D** (SRS 2026-09-05) — a database CHECK (`admin_is_srs_administrator`, migration `0036`), not a comment. A position is a *title*; an `org_role` is a *capability*; `user_roles` is the live answer to "what may this account do right now".

| Who — CBL position | Role | Read | Write | Notes |
|---|---|---|---|---|
| **Chief Executive Officer** (CEO) and **Chief Operations Officer** (COO) — CBL Art. III §2.1–2.2 | `exec_admin` | All org records, **all terms** | INSERT/UPDATE on all org records. **Sole** writer of `memberships.status='terminated'` (Art. VII §3.2.3, a majority Executive Board vote). Writer of every `officer_assignments.status` transition (Art. VI: the CEO approves LOA §1.2 and resignation §2.2; the Executive Board votes to impeach §3.2.7) — **no longer sole**: ADR 0012 (2026-09-06, migration 0046) widens this write to `crrd_admin` as well, as a second *recorder* of a decision Art. VI still reserves to the CEO or the Executive Board to *make*. **No** EXECUTE on `roll_over_term()` — see the callout below. | **No DELETE anywhere.** `memberships.status='terminated'` is the one place `exec_admin` is *wider* than `crrd_admin` on a records table; officer standing is no longer one, per ADR 0012. |
...
>
> ✅ **Restored 2026-09-05:** the SRS puts the DCTO-PD back in `tech_admin` (migration `0036`), so the seat has two occupants again and the paragraph below describes the 2026-09-01 state it replaces. ⚠️ **This mitigation got weaker on 2026-09-01 and had not been replaced.** An earlier draft leaned on DCTO-PD also holding `tech_admin`, giving the seat two occupants. The project heads have since scoped administrators to CEO, COO, CTO and the CCDO **only**, moving DCTO-PD to `moderator`. **The CTO is now the sole holder of the only role that can roll over a term.** If that seat is vacant at term boundary — the single most likely moment for it to be vacant — rollover is blocked and requires a migration to unblock. This needs a human decision (see PRD OQ-13), not a doc edit. The two candidate fixes are a break-glass `exec_admin` path, audited and alarmed, or a hard handover gate that forbids the outgoing CTO from vacating before granting. **The Constitution bears hard on this and changes what the options are — read "Vacancy, tenure and the rollover guard" immediately after this table before answering OQ-13.**
| **Chief Technology Officer** (CTO) — CBL Art. III §2.3, head of the Technology Department (Art. III §4.2) — **and the DCTO-PD** (Art. III §3.2) since 2026-09-05 | `tech_admin` | System config; **deliberately NOT blanket read of sensitive columns** | Only role that can write `user_roles`, `application_windows`, `terms`; EXECUTE `unfreeze_term()`; **sole** EXECUTE on `roll_over_term()` | PRD grants them "configure the system and control access", not "read everyone's address". Least privilege. Flagged in `openQuestions` — if read access is granted it must be a distinct audited role, not a quiet widening. |
| **Chief Community Development Officer** (CCDO) — CBL Art. III §2.7, head of the Community & Regional Relations Department (Art. III §4.6) — **with the DCCDO-C and DCCDO-D** (Art. III §3.9–3.10) since 2026-09-05, which the Constitution makes *"the primary bridge between the national organization and its regional chapters, members, and partner communities"* (Art. IV §6) | `crrd_admin` | All member/application records, **including every sensitive column** (project-head decision, 2026-09-01) | `people`, `memberships`, `committees`, `departments`, `applications`; EXECUTE `approve_application()` + `send_campaign()`; **sole issuer of `rr_send_grants`**; creates/renames committees and departments | The operational heart of the system. Full-column reads are audited **and gated on a current-term confidentiality acknowledgement** (CBL Art. VIII §7.1 — see below). Committee creation is free-form per Art. III §5; the seven departments are constitutionally fixed by Art. III §4, so that half of the power is amendment-paced — §4.4. |
| ~~**Deputy Chief Community Development Officer for Community** (DCCDO-C) and **for Development** (DCCDO-D) — CBL Art. III §3.9–3.10; **Deputy Chief Technology Officer for Product Development** (DCTO-PD) — Art. III §3.2~~ **Retired 2026-09-05 — the deputies are `crrd_admin` / `tech_admin` above; row kept for the record** | `moderator` (unassignable) | Same operational records and full columns — **you cannot review an application without reading it** | Review/approve/reject applications; update member records and status; assign members to **existing** committees and departments; compose and send campaigns | **Cannot** create or delete committees/departments, issue `rr_send_grants`, assign roles, configure the system, define terms, or roll over — **nor** flip a membership to `terminated` (CBL Art. VII §3.2.3) or touch any `officer_assignments.status` (Art. VI), both of which the Constitution reserves to the Executive Board. Structure, discipline and access control are chief-level. Full-column reads audited identically and gated on the same Art. VIII §7.1 acknowledgement. |
| Every other chief and deputy: **Chief Finance Officer** (CFO), **Chief Marketing Officer** (CMO), **Chief Communications Officer** (CCO), **Chief Events Officer** (CEvO) and the **Special Advisor** — CBL Art. III §2.4–2.6, §2.8–2.9; **DCOO** (Administrative Affairs), **DCTO-TE** (Tech Education), **DCFO-RMD**, **DCMO-SP**, **DCMO-CC**, **DCCO-P**, **DCCO-SMR**, **DCEvO-P**, **DCEvO-L** — Art. III §3 | `officer` | SELECT **only**, and only through `v_member_directory` | **None — no officer UPDATE policy exists on any table** | "View-only" is a property of the database, not of the UI. **Two rows here are counter-intuitive and deliberate.** *(Amended 2026-09-11, ADR 0019: the Special Advisor position is now retired from recording — `officer_positions.is_active = false`, no tier may write an assignment to it, and no screen lists it. The reasoning below is kept for the record.)* The *Special Advisor* sits with the Executive Board (Art. III §2.9) but is non-voting, is a DOST-SEI employee rather than a scholar (Art. X §3.1), and is the **independent** reviewer of appeals against disciplinary action (Art. X §2.4–2.5) — an advisor with `exec_admin` would be reviewing appeals against their own writes. The *DCOO* is the officer Art. VI §1.6 makes responsible for issuing a formal AWOL notice, yet holds no write access here; see the divergence flagged below. |
| **Regional Representatives** — CBL Art. III §4.6 (seated in CRRD) and Art. IV §6.4, serving *"under the supervision of the DCCDO-C"* | `regional_rep` | SELECT where `region_id = auth_region_id()`; **plus, since 2026-09-05 (ADR 0011), their own region's email / contact number / Facebook / university through `list_region_member_contacts()`** — a definer, acknowledgement-gated, one audit row per call; the GRANT and the view are unchanged | **Zero UPDATE and zero DELETE policies exist for this role on any table** | "RRs cannot delete or alter records" is a *missing policy*, not a missing button. Sending requires an unexpired `rr_send_grants` row — the PRD's "go signal by CRRD", issued only by the CCDO. One of the two multi-seat positions (18 regions), hence excluded from the single-occupancy index in DATA_MODEL.md §6/0007. |
...
⚠️ **One CBL/role-model divergence, flagged rather than silently fixed — and now partially resolved on its OPERATIONAL side.** CBL Art. VI §1.6 makes the **DCOO** the officer who issues the formal AWOL notice that leads to automatic dismissal from office (§1.7). The locked role model gives the DCOO `officer` — SELECT only. (Art. IV §1.3.5 does make the DCOO responsible for *"a centralized records management system"*, but for **meeting minutes, project documents and memoranda** — not membership records, so it does not pull membership write access along with it. That distinction is worth keeping explicit before someone reads §1.3.5 as a claim on START-SYS.) So the notice is still issued outside the system, but — since ADR 0012 (2026-09-06, migration 0046) — the resulting `officer_assignments.status = 'dismissed'` flip can now be recorded by `crrd_admin` as readily as by `exec_admin`, with the DCOO named in `status_note` either way. Granting the DCOO *write* access itself would still create a quiet fifth-and-sixth administrator, which the 2026-09-01 decision forecloses, and ADR 0012 does not do that — it widens the recorder tier, not the decider. **The DCOO's own write access remains a question for the project heads, not a doc edit** (DATA_MODEL.md §3.4; PRD OQ-16).
...
**Session hardening:** 1-hour access tokens, refresh rotation on, rate limiting on `/login` and `/apply` keyed on IP + email. **Idle logout after 1 hour with no activity, for every signed-in account, and no absolute session cap** (project head, 2026-09-11). The `start_sys_last_activity` cookie holds the last activity time: the browser rewrites it on input, `middleware.ts` on every non-prefetch request. `components/auth/idle-logout.tsx` (rendered by `AppShell`) warns one minute before with "Stay logged in"; `middleware.ts` independently signs the session out (`scope: 'local'`, this browser only, never global) once the value is more than an hour old, so a closed tab or a sleeping laptop cannot skip it. The cookie is not a credential — forging it only extends the forger's own session.
...
- Exact **row counts** and exact **visible column sets** per role fixture — nine fixtures: `anon`, `member` (the revoked tier), `officer`, **two different regional reps**, `crrd_deputy` (a second `crrd_admin` with **no** confidentiality acknowledgement — the day-one refusal case), `crrd_admin`, `exec_admin`, `tech_admin`.
