...
6. **The START-DOST Constitution and By-Laws 2026 (CBL) is the authority for org structure, tenure, separation from office and membership termination** — 23 positions (Art. III §2/§3/§4.6/§5), seven departments (Art. III §4), a term ending in May (Art. V §1). **Which position holds which access tier is the CRRD SRS's** ("Questions Roles and Features", 2026-09-05; ADR 0009): seven administrators — CEO, COO (`exec_admin`), CTO, DCTO-PD (`tech_admin`), CCDO, DCCDO-C, DCCDO-D (`crrd_admin`) — a database CHECK; no moderator tier; members hold no accounts and only submit forms. It is seeded data with articles cited inline, never a string literal in TypeScript. Never invent a provision; cite only what the text says.
