...
| 13 | `officer_positions` | global | **CBL** | The 23 positions of CBL Art. III §2 (C-Suite + Special Advisor), §3 (Deputy Board), §4.6 (Regional Representative) and §5 (Committee Member), and the `org_role` each grants. Seeded verbatim from the Constitution — see §6/0016. `is_active` (0063) retires a position from recording without deleting its row; the Special Advisor is retired (ADR 0019). |
...
| 31 | `universities` | global | SRS | Institutions by region. Seeded as a starter list (0037); since 0048 (2026-09-08) the rows are the DOST-SEI study-placement list — UP, SUCs, private HEIs with a CHED COE/COD or FAAP Level III+ in a priority program (`docs/issues/2026-09-07-dost-sei-eligible-institutions.md`). Local Universities and Colleges added by 0050 (2026-09-08) per the DOST-SEI helpdesk FAQ. CRRD edits; the RR view filters by it. |
| 32 | `psgc_locations` | global | **PSA** | The Philippine Standard Geographic Code as rows — 43,769 of them, region down to barangay, self-referencing (0057). What makes the application form's address four dropdowns instead of three free-text boxes. Anon-readable; written only by a migration. |
...
| Fact | Lives on | Why |
|---|---|---|
| Name, birthdate, contact number, **both addresses**, personal email, social profile links | `people` | Properties of a human, not of a membership. The four social links (0038, 0055) are contact channels and sit with the phone number. **Two addresses since 0058** — HOME (permanent) and CURRENT (where the scholar lives while studying) — each PSGC-coded plus the resolved names. ⚠ `region_name` on an address is NOT `memberships.region_id`: the latter is the ORG region, resolved from the university at approval and frozen, and it drives Regional Rep scoping. A scholar from Bicol studying in Manila has a Bicol home address and an NCR org region. |
...
|---|---|---|
| `people` | `birthdate`, `contact_number`, `personal_email`, `school`, `school_id_no`, `middle_name`, `facebook_account` (0038), `instagram_account`, `github_account`, `linkedin_account` (0055); **the HOME address** — `address_line`, `barangay`, `sub_municipality`, `city_municipality`, `province`, `region_name`, `postal_code`, `psgc_barangay_code`, `psgc_city_code`; **the CURRENT address** — the same nine, `current_`-prefixed (0058) | Directly identifying / contact / government-scholarship-linked. The four social links are contact channels treated exactly like `contact_number`. **The PSGC CODES are registered alongside the names on purpose**: a barangay code identifies a household as precisely as the barangay's name does, so masking the name and leaving the code beside it would be masking nothing. `current_address_same_as_home` is a boolean about a form and is deliberately NOT registered. |
...
8. **Reference data published by somebody else is a generated migration, not a hand-typed one.** `psgc_locations` (0057) comes from the PSA's quarterly workbook via `scripts/generate-psgc-migration.py`, with the publication date pinned in the migration header. A new quarter is a NEW generated migration, never an edit — so "which PSGC is this record's address from?" has an answer years later. Never hand-edit those 43,769 rows.
9. **Prefer a table over an enum for anything a CRRD officer might want to add** (affiliations, officer positions). Prefer an enum for anything fixed by law or geography (island groups, org roles).
10. **Anything the CBL fixes is seeded or CHECKed, never hard-coded in TypeScript.** The seven departments, the 23 positions, the four administrators and the May term boundary are all in `0003`/`0005`/`0016` with an article cited inline. When the Constitution is amended (Art. XII), the diff is a migration with a citation — not a grep for string literals across `app/`.
11. **Separation from OFFICE (`officer_assignments.status`, CBL Art. VI) and termination of MEMBERSHIP (`memberships.status`, CBL Art. VII) are different enums on purpose.** Before adding a value to either, check which Article your event lives in. An impeached officer is still a member; a terminated member holds no office. Merging them is the single most likely future mistake in this schema.
