...
**Every transition is `exec_admin`.** This follows from the locked role model, not from a reading of the CBL: `moderator` explicitly *cannot assign roles*, and `officer` is SELECT-only. It happens to line up with Art. VI, where the CEO or the Executive Board decides every one of these.
