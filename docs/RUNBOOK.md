...
2. Edit the subject and message. There are two tabs, and switching between them keeps both drafts.
   - **Write it here** — Telegram-style formatting: `**bold**`, `__underline__`, `_italic_`, `~~strike~~`, `` `code` ``, `[label](https://…)`, `![alt](https://image)`, lines starting `- ` or `1. ` for a list, `> ` for a quote, `#` / `##` / `###` for a heading, `---` for a divider. Up to 20,000 characters.
   - **Paste a design** — paste the HTML your email builder exports, the same code you would paste into Gmail through a template extension. Up to 200 KB. It is checked on the server before it is saved: scripts, forms, frames, event handlers and any address that is not `https` are removed, and the preview shows exactly what survived. **Two things a builder may include are dropped on purpose:** `<style>` blocks — phone-only layout tweaks, so a multi-column design shows its desktop layout on a phone, scaled to fit — and images that are not on an `https` address. Host images wherever your builder puts them and paste the link it gives you.

   Merge fields work the same on both tabs and are `{{given_name}}`, `{{family_name}}`, `{{member_id}}`, `{{join_year}}`, `{{region_name}}`, `{{island_group}}`, `{{term_label}}`, `{{year_level}}`, `{{committee_name}}`, `{{department_name}}` — nothing else can be merged, and an unknown token blocks saving.
