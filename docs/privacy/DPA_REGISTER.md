...
| 3 | Google (Drive) | Proof-of-enrollment document storage, `drive.file` scope only | The uploaded Certificate of Registration / scholar ID; never the database record | Google-managed, not disclosed per-file | Yes — Google Workspace/Cloud DPA, part of the Workspace terms | **NOT EXECUTED — and now ACTIVE.** Live since 2026-09-10 (ADR 0018), via an OAuth credential for the org's own Google account rather than a Workspace Shared Drive, so OQ-1 no longer gates it. Executing this DPA is now blocking, not contingent |
