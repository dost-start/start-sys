...
| 11 | `GMAIL_SMTP_APP_PASSWORD` (+ `GMAIL_SMTP_USER`) — ADR 0010 | Vercel env (Production only; Preview runs `MAIL_TRANSPORT=fake`) · Bitwarden. Generated by the **holder of the START community Gmail account**, never by the CTO. | Every campaign send fails with `misconfigured` (SMTP 535) until the new password is in Vercel and the functions restart; the queue keeps its rows and resumes | 1. Account holder: Google Account → Security → 2-Step Verification → App passwords → revoke "START-SYS", generate a new one named the same. 2. Hand it to the CTO out of band — never chat, never email. 3. CTO: update the Vercel Production env var and the Bitwarden item. 4. Redeploy. 5. Confirm via `/api/health` mail ping, then send one test message to yourself. **Never paste the password into the repository, a PR, an issue or a chat.** |
