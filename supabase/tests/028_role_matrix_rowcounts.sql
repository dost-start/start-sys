...
select is((select count(*) from public.sensitive_column_registry)::int,        26, 'exec_admin sees exactly 26 sensitive_column_registry rows — the map of where the PII is (18 + facebook_account + noa_drive_file_id + the three renewal_submissions pointers of 0044 + the three optional social links of 0055)');
...
select is((select count(*) from public.sensitive_column_registry)::int,        26, 'tech_admin sees exactly 26 sensitive_column_registry rows');
