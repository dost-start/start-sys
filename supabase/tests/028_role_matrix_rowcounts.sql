...
select is((select count(*) from public.sensitive_column_registry)::int,        40, 'exec_admin sees exactly 40 sensitive_column_registry rows — the map of where the PII is (18 + facebook_account + noa_drive_file_id + the three renewal_submissions pointers of 0044 + the three optional social links of 0055 + the fourteen address columns of 0058)');
...
select is((select count(*) from public.sensitive_column_registry)::int,        40, 'tech_admin sees exactly 40 sensitive_column_registry rows');
