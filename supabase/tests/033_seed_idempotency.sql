...
select is((select count(*)::int from public.sensitive_column_registry), 18,
  'BEFORE: 18 sensitive_column_registry rows — the RA 10173 classification, as data (CBL Art. VIII §6)');
...
select is((select count(*)::int from public.sensitive_column_registry), 18,
  'AFTER: still 18 registry rows — a duplicated classification would double every audit mask lookup');
