...
-- 7 — the digest is the link between the row and the bytes. The CURRENT version (v4, 0061)
-- carries the sha256 of docs/privacy/PRIVACY_NOTICE.md as committed; v1, v2 and v3 keep the
-- digests of the bytes THEIR applicants agreed to, which the file no longer has. v3 is
-- asserted below precisely BECAUSE it is superseded: a past version's digest must never
-- drift, or the record of what someone agreed to stops being a record.
...
select is(
  ...
  'the superseded v3 digest is UNCHANGED — the applicants who consented under v3 consented '
